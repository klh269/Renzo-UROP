...
for i in tqdm(range(galaxy_count)):
    ...
    D, e_D = table["D"][i], table["e_D"][i] # Distance to galaxy
    
    # Propagate errors (1 sigma) from D and M/L for errVbar.
    def errVbar(arr):
        v_sq = arr["Vgas"]**2 + arr["Vdisk"]**2*(pdisk+0.125) + arr["Vbul"]**2*(pbul+0.175)
        v_sq *= (D+e_D) / D
        errVbar = np.sqrt(v_sq) - Vbar(arr)
        return errVbar
    ...
    Vobs = interpolate.UnivariateSpline(r, nVobs, k=3)
    Vbar_fit = interpolate.UnivariateSpline(r, nVbar, k=3)
    Vgas = interpolate.UnivariateSpline(r, nVgas, k=3)
    Vdisk = interpolate.UnivariateSpline(r, nVdisk, k=3)
    if bulged:
        Vbul = interpolate.UnivariateSpline(r, nVbul, k=3)
    
    # Compute residuals.
    res_obs = (Vobs(r) - nVobs) / nerrV
    res_bar = (Vbar_fit(r) - nVbar) / nerrVbar
    
    # Fit quartic spline with s=0 through residuals.
    rf_obs = interpolate.UnivariateSpline(r, res_obs, k=4, s=0)
    rf_bar = interpolate.UnivariateSpline(r, res_bar, k=4, s=0)
    
    """
    Compute finite differences between residuals.
    """
    if finite_diff:
        dr = np.diff(r)
        dVobs_dr = np.diff(rf_obs(r)) / dr
        dVbar_dr = np.diff(rf_bar(r)) / dr
        d2Vobs_dr2 = np.diff(dVobs_dr) / dr[1:]
        d2Vbar_dr2 = np.diff(dVbar_dr) / dr[1:]
    ...
    if plot_splines:
        ...
        plt.errorbar(r, nVobs, yerr=nerrV, color='k', fmt="o", capsize=3, alpha=0.3)
        ...
        plt.plot(rad, rf_obs(rad), color='k', label="Vobs")
        plt.plot(rad, rf_bar(rad), color='red', label="Vbar")
        plt.scatter(r, res_obs, color='k', alpha=0.3)
        plt.scatter(r, res_bar, color='red', alpha=0.3)
        plt.legend(bbox_to_anchor=(1,1), loc="upper left")
        plt.grid()
        
        plt.show()
        fig0.savefig("C:/Users/admin/OneDrive/Desktop/Other/Oxford UROP 2024/plots/Vbar_Vobs/residuals/"+g+".png", dpi=300, bbox_inches="tight")
        plt.close()
        
        # Plot finite differences of residuals.
        fig1, ax1 = plt.subplots()
        ax1.set_title("Finite difference: "+g)
        
        color = "tab:red"
        ax1.set_xlabel(r"Normalised radius ($\times R_{eff}$)")
        ax1.set_ylabel(r'$dv_{bar}/dr$', color=color)
    
        ln1 = ax1.plot(r[1:], dVbar_dr, color=color, label="Baryonic curve - Vbar")
        ax1.tick_params(axis='y', labelcolor=color)
        
        ax2 = ax1.twinx()
        color = "black"
        ax2.set_ylabel(r'$dv_{obs}/dr$', color=color)
        ln2 = ax2.plot(r[1:], dVobs_dr, color=color, label="Total curve - Vobs")
    ...
    #     plt.savefig("C:/Users/admin/OneDrive/Desktop/Other/Oxford UROP 2024/plots/Vbar_Vobs/corr_R/"+g+".png", dpi=300, bbox_inches="tight")
    #     plt.show()
    #     plt.close()
    ...
    #     plt.savefig("C:/Users/admin/OneDrive/Desktop/Other/Oxford UROP 2024/plots/Vbar_Vobs/corr_V/"+g+".png", dpi=300, bbox_inches="tight")
    #     plt.show()
    #     plt.close()
