    ...
    if do_DTW:
        ...
        for n in range(len(r)):
            for m in range(len(r)):
                dist_data[n, m] = abs(res_Vobs[n] - res_MOND[m])
                dist_LCDM[n, m] = abs(res_LCDM[n] - res_MOND[m])
        
        dist_mats = np.array([ dist_data, dist_LCDM ])
        mats_dir = [ "data/", "LCDM/" ]
...
if __name__ == "__main__":
    ...
    for i in range(galaxy_count):
        ...
        v_LCDM = np.sqrt(Vbar2 + np.array(v_DM[i])**2)
