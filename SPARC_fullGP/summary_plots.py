...
if __name__ == "__main__":
    ...
    for g in galaxies:
        ...
        if len(Vbar_features["lb"]) > 0:
            print(f"Galaxy {g} has features in Vbar: {Vbar_features}")
            Vbar_ft += 1
        if len(Vobs_features["lb"]) > 0:
            print(f"Galaxy {g} has features in Vobs: {Vobs_features}")
            Vobs_ft += 1
        print("")
    print(f"No. of galaxies with features in Vbar: {Vbar_ft}")
    print(f"No. of galaxies with features in Vobs: {Vobs_ft}")
    ...
    sort_args = np.argsort(dtw_percentiles[2][0])
    ...
    ax.plot(galaxies[sort_args], dtw_percentiles[2][0], color='k', label="Data")
    ...
    for j in range(1, 3):
        ...
        ax.errorbar(galaxies[sort_args], dtw_percentiles[2][j], [low_err, up_err], fmt='.', ls='none',
