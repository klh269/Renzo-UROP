...
plot_digitizer = True   # True (False) == Sanders (Gentile et al.)
use_window = False   # Use only a window around feature, [16:25] or [32:48], for analysis.
...
# Main code to run.
def main(args, r_full, rad, v_data, v_mock, num_samples=num_samples, redo_GPR:bool=False, ls:float=4.5):
    """
    ...
    delta_vcomps = [ r"$\delta V_{\text{bar}}$", r"$\delta V_{\text{obs}}$", r"$\delta V_{\text{MOND}}$", r"$\delta V_{\Lambda CDM}$" ]
    ...
    Vbar_errors_full = ( np.diff(raw_percentiles[:,0], axis=0) / 2 )[0]
    ...
    for j in range(2):
        ...
        if plot_digitizer:
            ...
            r_Xft = np.delete(r_full, np.s_[37:48], axis=0)
            data_Xft = np.delete(v_data[j], np.s_[37:48], axis=0)
            means, _ = vmap(
                lambda rng_key, var, noise: predict(
                    rng_key, r_Xft, data_Xft, rad, var, ls, noise, use_cholesky=args.use_cholesky
                )
    ...
    print(f"Shape of res_data: {res_data.shape}\n")
    ...
    if redo_GPR:
        ...
        for smp in range(num_samples):
            ...
            for j in range(4):
                ...
                if plot_digitizer:
                    r_Xft = np.delete(r_full, np.s_[16:25], axis=0)
                    mock_Xft = np.delete(v_mock[j,smp], np.s_[16:25], axis=0)
                    means, _ = vmap(
                        lambda rng_key, var, noise: predict(
                    ...
                    r_Xft = np.delete(r_full, np.s_[32:48], axis=0)
                    mock_Xft = np.delete(v_mock[j,smp], np.s_[32:48], axis=0)
                    means, _ = vmap(
                        lambda rng_key, var, noise: predict(
                            rng_key, r_Xft, mock_Xft, rad, var, ls, noise, use_cholesky=args.use_cholesky
                        )
        ...
        if plot_digitizer:
            np.save("/mnt/users/koe/mock_residuals/NGC1560_Sanders.npy", res_mock)
            np.save(f"/mnt/users/koe/mock_residuals/plotGP/NGC1560_Sanders.npy", plotGP_mock)
        else:
            np.save("/mnt/users/koe/mock_residuals/NGC1560_Gentile.npy", res_mock)
            np.save(f"/mnt/users/koe/mock_residuals/plotGP/NGC1560_Gentile.npy", plotGP_mock)
        ...
        if plot_digitizer:
            res_mock = np.load("/mnt/users/koe/mock_residuals/NGC1560_Sanders.npy")
            plotGP_mock = np.load(f"/mnt/users/koe/mock_residuals/plotGP/NGC1560_Sanders.npy")
        else:
            res_mock = np.load("/mnt/users/koe/mock_residuals/NGC1560_Gentile.npy")
            plotGP_mock = np.load(f"/mnt/users/koe/mock_residuals/plotGP/NGC1560_Gentile.npy")
        
        print("Mock residuals and GP means loaded.")

    print(f"Shape of res_mock: {res_mock.shape}\n")

    if use_window:
        if plot_digitizer:
            res_data = res_data[:,16:25]
            res_mock = res_mock[:,16:25]
            err_Vobs = v_data[2][16:25]
            Vbar_errors = Vbar_errors_full[16:25]
            r = r_full[16:25]
        else:
            res_data = res_data[:,32:48]
            res_mock = res_mock[:,32:48]
            err_Vobs = v_data[2][32:48]
            Vbar_errors = Vbar_errors_full[32:48]
            r = r_full[32:48]
    else:
        r = r_full
        err_Vobs = v_data[2]
        Vbar_errors = Vbar_errors_full
            ...
            for j in range(3):
                ...
                if make_plots and smp == 0:
                    ...
                    if j == 0:
                        ...
                        plt.plot(np.arange(len(r)), res_data[1] + diff, c='k', label=delta_vcomps[1])
                        plt.plot(np.arange(len(r)), res_data[0] - diff, c="tab:red", label=r'$\delta V_{\text{bar}}$')
                        ...
                        plt.plot(np.arange(len(r)), np.array(res_mock)[j+1,:,smp] + diff, c=colours[j+1], label=delta_vcomps[j+1])
                        plt.plot(np.arange(len(r)), np.array(res_mock)[j-1,:,smp] - diff, c='tab:red', label=r'$\delta V_{\text{bar}}$')
                    ...
                    plt.savefig(fname_DTW+"vis_"+mats_dir[j]+".pdf", dpi=300, bbox_inches="tight")
                    plt.close('all')
    ...
    if do_correlations:
        ...
        """
        Plot GP fits, residuals and correlations (as 3 subplots in one fig).
        """
        if make_plots:
            ...
            for j in range(4):
                ...
                elif j == 0: ax0.errorbar(r_full, v_data[0], Vbar_errors_full, color='tab:red', alpha=0.3, fmt='.', capsize=2.5)   # Vbar
            ...
            if plot_digitizer:
                ax0.axvline(r_full[16], c='k', ls='--', alpha=0.5, zorder=0, label="Feature")
                ax0.axvline(r_full[25], c='k', ls='--', alpha=0.5, zorder=0)
            else:
                ax0.axvline(r_full[32], c='k', ls='--', alpha=0.5, zorder=0, label="Feature")
                ax0.axvline(r_full[48], c='k', ls='--', alpha=0.5, zorder=0)
            ...
            if plot_digitizer:
                ax1.axvline(r_full[16], c='k', ls='--', alpha=0.5, zorder=0, label="Feature")
                ax1.axvline(r_full[25], c='k', ls='--', alpha=0.5, zorder=0)
            else:
                ax1.axvline(r_full[32], c='k', ls='--', alpha=0.5, zorder=0, label="Feature")
                ax1.axvline(r_full[48], c='k', ls='--', alpha=0.5, zorder=0)
            ...
            if plot_digitizer:
                ax2.axvline(r_full[16], c='k', ls='--', alpha=0.5, zorder=0, label="Feature")
                ax2.axvline(r_full[25], c='k', ls='--', alpha=0.5, zorder=0)
            else:
                ax2.axvline(r_full[32], c='k', ls='--', alpha=0.5, zorder=0, label="Feature")
                ax2.axvline(r_full[48], c='k', ls='--', alpha=0.5, zorder=0)
...
if __name__ == "__main__":
    ...
    if plot_digitizer: print("\nAnalyzing NGC 1560 from Sanders (2007) with")
    else: print("\nAnalyzing NGC 1560 from Gentile et al. (2010) with")
    if use_window: print("windowed data around 4.2 - 6.2 kpc.")
    else: print("full RCs.")
    ...
    main(args, r.to_numpy(), rad, v_data, v_mock, redo_GPR=args.redo_GPR)
...
if make_plots:
    ...
    if do_correlations:
        ...
        for j in range(2):
            ...
            print("Lower error = {:.4f}".format(low_err[0]))
            print("Upper error = {:.4f}".format(up_err[0]))
